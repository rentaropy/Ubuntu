<#
.SYNOPSIS
    SSH Key & Git Environment Setup Script (Simple Permission Fix)
.DESCRIPTION
        ...
        if (Get-UserConfirmation "SSH鍵(.ssh)をWindowsからWSLに同期しますか？") {
            ...
            if (-not (Test-Path $srcPath)) {
                ...
                try {
                    ...
                    # 2. ファイル一括権限 (600)
                    # 公開鍵も600になりますが、動作上の問題はなく、最も安全で確実な方法です
                    wsl -d $distroName chmod 600 ~/.ssh/*

                    Write-Host "SSH鍵の権限を修正しました (Distro: $distroName, Path: ~/.ssh/*)。" -ForegroundColor Green
                } catch {
